from screen_eat.exception import AuthError
...
class ImgurPrivateUploader(Uploader):
    ...
    def headers(self):
        return {"authorization": "Bearer {0}".format(self.auth.data["access_token"])}

    def isConfigured(self):
        return (
            self.auth.data["client_id"] != "" and self.auth.data["client_secret"] != ""
        )

    def isAuthenticated(self):
        return (
            self.auth.data["access_token"] != ""
            and self.auth.data["refresh_token"] != ""
        )
    ...
    def getAccessToken(self, pin):
        ...
        payload = {
            "client_id": self.auth.data["client_id"],
